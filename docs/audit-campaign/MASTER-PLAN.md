...
- ✅ T1.3 — Installer + release.ps1 audit (2026-05-29: findings/T1.3-installer.md — **22 findings: 0 P0, 4 P1, 13 P2, 5 P3** + 8 verified-clean/informational. Run under the `code-review` + `superpowers:systematic-debugging` skills per the Phase-1 directive — 9 finder angles + verify/sweep executed **inline** (no unsolicited subagents). All 17 `Source:` + 4 compile-input files exist — **no missing-file P0 build break**; `tests/` confirmed excluded by omission. The 4 P1s: installer ships the dev's **live config.json** (DPAPI groqApiKey + `launchAtStartup=1`) → on fresh install the ~240-char blob trips onboarding's "key set" heuristic (`onboarding_ui.ahk:182`) so `OnClose` **marks setup done**, but it can't be decrypted on the user's box (`dpapi.ahk:113`→"") → **transcription silently fails on first use, wizard never re-prompts** — confirmed first-run break (001); `release.ps1` bumps ~7 source files **before** compile/sign and **never commits the app-source bumps** → half-done run leaves a dirty tree, no rollback (011); **user data lives under `{app}` (`%LOCALAPPDATA%\Programs\QuickSay Beta\`), NOT `%APPDATA%\QuickSay\`** → the skill/T2.3 invariant is false, trial.dat/license.dat location must be decided in T2.1 (023); app writes `HKCU\…\Run\QuickSay`→`{app}\QuickSay.exe` that the uninstaller never removes → **orphaned autorun to a deleted exe** (025). **`release.ps1 --check-sync` spec (T1.3-015): 23-location version list** handed to T1.6 — incl. the never-synced `config.json lastSeenVersion` and the patch-release filename collision (`vX.Y` short → 1.9.0/1.9.1 overwrite same R2 object). **Skill-pass adds:** patch releases silently **skip the GitHub upload** (STEP 8 uses `v$semVer` filename w/o the fallback STEP 4/5b have, 029); version propagation is **warn-only** — a drifted regex ships a stale version → `--check-sync` must *assert*, not warn (030). Azure sign-hang: no pre-sign `codesigning.azure.net` token probe + the auto-`az login` fallback omits `--scope` — recipe surfaced for T1.6 (016). Signing: QuickSay.exe + QuickSay-Setup.exe + installer all Valid/consistent (`CN=Adrian Beeksma`), but **uninstaller is unsigned** (no `SignedUninstaller`) (017); manifest declares legacy `dpiAware=true`, not PerMonitorV2 (021, cross-ref T1.7). WebView2 redist is the 1.6 MB Evergreen **bootstrapper** (needs internet; ResultCode unchecked → silent fail offline, 027). **Corrects P0.2:** `signing/metadata.json` has NO `Version` field (019). M.2 clean-VM procedure authored push-button (not executed — no VM); flags the fresh-vs-upgrade `onlyifdoesntexist` uninstall nuance to confirm on-VM. Cross-deps: T1.6 (most fixes + --check-sync + 029/030), T2.1/T2.3 (data location), M.1 (default config, Run-key cleanup, WebView2 offline), T1.7 (DPI), T1.4 (soundTheme="default" has no dir). Zero source changes; all git work in an isolated worktree.)
