...
- ✅ T1.8 — Track-1 audit P1 remediation (2026-06-05: fixed the 5 unactioned P1s from T1.3+T1.4, each with a regression guard. **Base:** branched off the Track-2 tip `audit/T2.7-telemetry` — T1.6+T1.7 were already merged into that line (PR #12), so no cherry-pick needed (topology note in findings §0). **#1 T1.3-023 (headline):** new `lib/datadir.ahk` `GetDataDir()` resolver — `%APPDATA%\QuickSay\` when compiled (co-located with T2.3's `license.dat`; trial state lives *inside* `license.dat`, no separate `trial.dat`), `A_ScriptDir` in dev; routed config/dictionary/history/stats/audio/logs/marker + ~53 debug sites through it across tray/settings/onboarding (no split-brain); bundled `changelog.json` stays install-relative; one-time **non-destructive, idempotent, move-only** migration from legacy `{app}` + clean-template seed (`BootstrapDataDir()`); `setup.iss` `[Dirs]`→`%APPDATA%`, `OnboardingAlreadyDone` checks new+legacy marker, uninstall prompt **preserves `license.dat`** (trial not resettable). No change to `lib/license.ahk` (already canonical). **#2 T1.3-001:** installer stops shipping the dev's live `config.json`/`dictionary.json` (DPAPI key + `launchAtStartup=1`); ships clean `config.example.json` template (fixed `soundTheme default→subtle`, T1.4-019). **#4 T1.3-025:** `[UninstallRun]` `reg delete`s the orphaned `HKCU\…\Run\QuickSay` value (not app-on-exit — that'd defeat autorun). **#3 T1.3-011:** new `scripts/release-rollback.ps1` + `try/finally` in `release.ps1` snapshots (DRY from T1.6's `$VersionTargets`) and **restores source byte-for-byte on any mid-run failure**; built on T1.6, wraps T2.5's STEP 6. **#5 T1.4-025:** `AddToDictionary` now calls `CompileDictionaryPattern()` → learned word applies to the next transcription. **Independent `security-auditor` on #1+#2: secure, no must-fix** (1 P3 fixed inline: `--settings` path now bootstraps). **Verified:** 4 new suites green (datadir 22, installer-hygiene 26, release rollback, dictionary guard — red-on-pre-fix); existing license/update/crash/telemetry **no regression**; `-CheckSync` 22/22, `-DryRun` clean. Findings: `findings/T1.8-p1-remediation.md`. Follow-ups for M.1: single-item `-Changelog` crash (`@($items).Count`), `AddToDictionary` no-mutex (T1.4-026). Dev branch `audit/T1.8-audit-p1-remediation`.)

### Phase 2
- ✅ M.1 — Wire systems → v2.0.0-rc1 (2026-06-06: all 13 scattered branches consolidated onto `audit/M.1-integration`. **Topology correction:** the session-doc's Phase-0 table was stale — the rich line was **`audit/T1.8-audit-p1-remediation`** (a strict superset of `T2.7-telemetry` + the 2 T1.8 commits; already contained main=T1.6+T1.7+sound, T2.3/T2.4/T2.5/T2.6/T2.7, and `release.ps1` with all 3 behaviors — check-sync+Ed25519+rollback already coexisting, no 3-way conflict), NOT the near-empty `audit/T1.8-findings`. Assembled by branching off that tip, cherry-picking the un-propagated T2.6 fixture fix (`990cc23`; the T2.4 JWT-scrub follow-up `c3e44ac` was redundant — already re-landed via `6288e5a`), merging `audit/T1.5-history-retention-fix` (lib/history-core.ahk + config-defaults; the one real conflict — resolved the QuickSay.ahk startup/recording seam: BootstrapDataDir + 0x5556 HistoryCleared coexist, paywall gate + RecordingGeneration coexist, context_aware_modes→false + crash keys kept, SSOT version preserved; dropped T1.5's superseded synthetic transcription fixtures in favor of T2.6's corpus), bringing in `Backend/license-worker/` from T2.2, and dropping no data. Per user decision, audit docs stay in the root repo (this file) — the integration branch / PR-to-main is production-code-only. **Seam map (all 4 resolved by the assembly, zero Phase-2 glue needed):** startup-ordering, single recording gate, signed-update-vs-version-sync, crash+telemetry emit (no double-emit, both off-by-default → fresh install sends nothing). **Gates green:** `-CheckSync` 22/22 @ 2.0.0; transcription regression 20/20 (WER clean 0.0% / accents 2.3%, hallucination filter intact, key-leak clean); Playwright settings+onboarding smoke pass (paywall render-ready + logic via license suite); unit suites all green — history 19, license 6 (+live staging), crash 36, telemetry 43, update 26+roundtrip, datadir 22, multimon 13, hotkey 6, rollback/dictionary/installer-hygiene. **Built v2.0.0-rc1:** VERSION→2.0.0 (strict 3-part semver; rc1 tracked via branch+artifact name per T1.6 regime), QuickSay.exe + QuickSay-Setup.exe compiled (FileVer 2.0.0.0) + **Azure-signed**, `installer/QuickSay_Beta_v2.0_Setup.exe` (30.5 MB) **Azure-signed + verified**, `version.json` **Ed25519-signed** (qs-2026, local artifact — NOT published to R2). Non-intrusive smokes pass: update accept(genuine)/reject(tampered, fail-closed) against the real rc1 manifest; crash envelope scrub proven by the crash suite on this build. **App points at STAGING** (`license-staging.quicksay.app`); prod cutover is M.3. **Deferred to M.2 (user-approved):** live install + dictation + paywall-activation smokes — rc1 NOT installed on the dev box, real install untouched. **rc1 NOT published** (no R2 upload, no GitHub release, website untouched). Dev branch `audit/M.1-integration`, PR to main. Build-env note for M.3: `release.ps1`'s hardcoded `$ahk2base` (`C:\Program Files\AutoHotkey\v2\AutoHotkey64.exe`) fails as an Ahk2Exe base on this machine — the repo's own `AutoHotkey64.exe` works; release.ps1 full pipeline also publishes (R2/website) with no internal-only flag.)
- ✅ M.2 — UAT **script written** (2026-06-06: `uat-2.0.0-rc1.md` — 14-item manual checklist + pre-flight + result-summary/sign-off, in the exact per-item block format, ordered for natural VM state-build (install→use-during-trial→expiry/paywall/activation→uninstall). Pinned from the shipped rc1: binary `localVersion`/`VERSION`=2.0.0 (rc1 via branch+artifact name), installer `QuickSay_Beta_v2.0_Setup.exe` (Azure-signed), app points at **STAGING** `license-staging.quicksay.app`, license/trial in DPAPI `%APPDATA%\QuickSay\license.dat`, day-11 countdown banner, blocking-but-closeable paywall (U1), terminal paste=Shift+Insert, uninstall **preserves** `%APPDATA%\QuickSay\` incl. license.dat. **🔴 Surfaced blocker for the user before running:** Item 13 (activation) cannot PASS until the staging LemonSqueezy **test store is provisioned** — the Worker's `LEMONSQUEEZY_API_KEY` is still a placeholder and **no test license key was ever recorded** (T2.2 README `<FILL IN>`), so `/activate` returns 403 for any key → provision-or-waive. **🟡** Mic-dependent items (4–9,11) need a real mic → Windows Sandbox has no audio input; use Hyper-V with mic pass-through or a clean local user account on hardware. `[CONFIRM]` items: onboarding key-step wording, Settings history-retention control existence, staging test key, uninstaller has no "remove my data" prompt in rc1. **Gate semantics documented (PASS or written WAIVER, else launch blocked).** **The gate is NOT satisfied — only the user's execution on a fresh VM can satisfy it.** Branch `audit/M.2-uat`, PR to main.)
- ⏸️ M.3 — Launch (**DEFERRED 2026-07-13 by user decision** — enhance-first: the E-series below runs to completion, then the UAT gate (against rc2, via E.5) and M.3 execute. The M.3 session doc remains valid; its UAT-gate reference becomes `uat-2.0.0-rc2.md`. Price re-confirmed by user 2026-07-13: **$39 first 500 → $74**.)

### Phase 3 — E-series: pre-launch enhancement (added 2026-07-13)

> **Origin:** M.3 gate-check session, 2026-07-13. User decision (grilled + locked): **enhance first, then launch**; quality over features; Mac version explicitly deferred until the Windows version is perfected. **Positioning locked:** match Wispr Flow / Aqua Voice on core-loop quality (accuracy, latency, formatting, polish), win on ownership ($39-once, no account, no cloud, private by default) — their cloud features are noted-and-skipped, not gaps. **Timebox: ~3–4 weeks.** Key evidence driving E.2: history.json stores `rawText` + `cleanedText` pairs (live 732 KB at `%APPDATA%\QuickSay\data\`, legacy 1.6 GB pre-T1.5 file at `%LOCALAPPDATA%\Programs\QuickSay Beta\data\`); user-reported cleanup-stage bug ("answers my question" / "random trailing yes"); Whisper call sends no dictionary-biasing `prompt` param today.

| ID | Session | Model | Effort | Parallel | Doc |
|---|---|---|---|---|---|
| E.1 | Hands-on competitive teardown (Wispr Flow + Aqua Voice, user present) | Opus 4.8 | high | ∥ E.2 | `sessions/session-E.1-competitive-teardown.md` |
| E.2 | Transcription quality lab (history mining → cleanup fix → dictionary biasing → dogfood instrumentation) | Opus 4.8 | xhigh | ∥ E.1 | `sessions/session-E.2-transcription-lab.md` |
| E.3 | Targeted real-world bug sweep + dogfood harvest (14-seam matrix) | Opus 4.8 | high | after E.2 | `sessions/session-E.3-bug-sweep.md` |
| E.4 | Benchmark-driven visual polish (widget/onboarding/paywall/settings) | Opus 4.8 | high | after E.1+E.3 | `sessions/session-E.4-visual-polish.md` |
