...
- ✅ T1.4 — Onboarding + widget + sound + dictionary audit (2026-05-29: findings/T1.4-onboarding-widget-sound-dict.md — **16 findings: 0 P0, 1 P1, 8 P2, 7 P3** + 11 verified-clean. The 1 P1: "Learn from Selection" reports success but never recompiles the dictionary pattern, so the learned word is inert until the next 0x5555 reload/restart (025). Six required verdicts: drag-fix HELD (5px threshold intact, widget-overlay.ahk:341); mouse-capture always released → NOT a P0; 18 sound files all present + valid RIFF/WAVE; **dictionary terms ARE regex-escaped → SAFE, no ReDoS/injection** (proven via isolated AHK PCRE run — c++→c\+\+, a.*b literal, metachar keys no-throw); onboarding API key DPAPI-encrypted + never logged; a11y/DPI tagged T1.7. Key P2s: `mechanical` installed but omitted from settings dropdown (017, verifies T1.2-024); `default` theme relies on top-level sounds\*.wav present in installed app but MISSING from Development tree → next build regresses default audio to SoundBeep (019, cross-ref T1.3); `\b…\b` anchoring silently kills dict entries with leading/trailing non-word chars like c++/c#/.net (023); ParseDictionaryJson is a brittle regex parser not JSON.Parse (024); AddToDictionary + HideAndUpdateConfig write files without the config mutex (026/016, cross-ref T1.6). Cross-deps: T1.2 (dropdown HTML), T1.3 (build inputs), T1.6 (no-mutex writes), T1.7 (DPI/multimon). Live GUI walkthroughs (Playwright wizard, physical widget drag) NOT run in this autonomous read-only pass — owed to follow-up: confirm "Skip for now" window.close() no-op (002) + 2px/10px drag. Zero source changes.)
