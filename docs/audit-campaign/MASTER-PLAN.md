...
- ✅ T1.3 — Installer + release.ps1 audit (2026-05-29: findings/T1.3-installer.md — **20 findings: 0 P0, 4 P1, 11 P2, 5 P3** + 8 verified-clean/informational. All 17 `Source:` + 4 compile-input files exist — **no missing-file P0 build break**; `tests/` confirmed excluded by omission. The 4 P1s: installer ships the dev's **live config.json** (encrypted groqApiKey + `launchAtStartup=1`) → fresh installs inherit it, onboarding doesn't reset (001); `release.ps1` bumps ~7 source files **before** compile/sign and **never commits the app-source bumps** → half-done run leaves a dirty tree, no rollback (011); **user data lives under `{app}` (`%LOCALAPPDATA%\Programs\QuickSay Beta\`), NOT `%APPDATA%\QuickSay\`** → the skill/T2.3 invariant is false, trial.dat/license.dat location must be decided in T2.1 (023); app writes `HKCU\…\Run\QuickSay`→`{app}\QuickSay.exe` that the uninstaller never removes → **orphaned autorun to a deleted exe** (025). **`release.ps1 --check-sync` spec (T1.3-015): 23-location version list** handed to T1.6 — incl. the never-synced `config.json lastSeenVersion` and the patch-release filename collision (`vX.Y` short → 1.9.0/1.9.1 overwrite same R2 object). Azure sign-hang: no pre-sign `codesigning.azure.net` token probe — recipe surfaced for T1.6. Signing: QuickSay.exe + QuickSay-Setup.exe + installer all Valid/consistent (`CN=Adrian Beeksma`), but **uninstaller is unsigned** (no `SignedUninstaller`) (017); manifest declares legacy `dpiAware=true`, not PerMonitorV2 (021, cross-ref T1.7). WebView2 redist is the 1.6 MB Evergreen **bootstrapper** (needs internet; ResultCode unchecked → silent fail offline, 027). **Corrects P0.2:** `signing/metadata.json` has NO `Version` field (019). M.2 clean-VM procedure authored push-button (not executed — no VM). Cross-deps: T1.6 (most fixes + --check-sync), T2.1/T2.3 (data location), M.1 (default config, Run-key cleanup, WebView2 offline), T1.7 (DPI), T1.4 (soundTheme="default" has no dir). Zero source changes; all git work in an isolated worktree.)
